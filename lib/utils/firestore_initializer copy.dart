...
class FirestoreInitializer {
  ...
  // ajout nouvelle pour les nouvelles configurations
  // Ajoutez ces méthodes à votre FirestoreInitializer

  Future<void> initializeAdminCollections() async {
    print('Initialisation des collections admin...');

    // Créer des documents en attente
    List<Map<String, dynamic>> pendingDocs = [
      {
        'title': 'Diplôme Licence Informatique',
        'description': 'Demande de diplôme pour Mama Seck',
        'studentName': 'Mama Seck',
      ...
      {
        'title': 'Attestation de réussite Master',
        'description': 'Attestation pour Fatou Sarr',
        'studentName': 'Fatou Sarr',
    ...
    for (var doc in pendingDocs) {
      await _firestore.collection('documents').add(doc);
    }

    // Créer collections UFR et départements
    final ufrs = ['UFR SAT', 'UFR SES', 'UFR LSH', 'UFR Médecine'];
    for (var ufr in ufrs) {
      await _firestore.collection('ufrs').add({'name': ufr});
    }
    ...
    for (var dept in departments) {
      await _firestore.collection('departments').add({'name': dept});
    }

    print('Collections admin initialisées');
  }
}
